...
posun (a,q, s,f, p) x = (a, q+x, s+x, map(+x) f, [(p1, p2+x, p3+x) | (p1,p2,p3) <- p ])

preved:: Reg -> Automat
preved (Hod x) = ([x], 2, 0, [1], [(x,0,1)])
preved (Iter x) = let (a,q,s, f, p) = (posun (preved x) 1)
                   in (a, q, 0, [0], ('E', 0, s):p ++ [('E', f1, 0) |f1 <- f])
preved (Zret x y) =  let (xa, xq, xs, xf, xp) = preved x 
                     in  let (ya, yq,ys, yf,yp) = posun (preved y) xq
                     in (unique ya xa, yq, xs, yf, xp ++ yp ++ [('E',xf1,ys)| xf1<-xf] )
preved (Sjed x y) =  let (xa, xq, xs, xf, xp) = posun (preved x) 1
                     in  let (ya, yq,ys, yf,yp) = posun (preved y) xq
                     in (unique ya xa, yq, 0, xf ++ yf, xp ++ yp ++ [('E',0,xs),('E',0,ys)] )
